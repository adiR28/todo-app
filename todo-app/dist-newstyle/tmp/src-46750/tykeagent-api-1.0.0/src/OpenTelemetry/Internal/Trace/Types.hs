...
{- | An identifier for the library that provides the instrumentation for a given Instrumented Library.
 ...
 If there is no established name, the recommendation is to prefix packages with "tykeagent-instrumentation", followed by the instrumented library name itself.
...
{- | A 'Span' is the fundamental type you'll work with to trace your systems.
 ...
 A trace is made up of multiple spans. Tracing vendors such as Zipkin, Jaeger, Datadog, Lightstep, etc. use the metadata from each span to reconstruct the relationships between them and generate a trace diagram.
-}
data Span
