...
{- |
 ...
 To create and manage 'Span's in OpenTelemetry, the <https://hackage.haskell.org/package/tykeagent-api OpenTelemetry API> provides the tracer interface. This object is responsible for tracking the active span in your process, and allows you to access the current span in order to perform operations on it such as adding attributes, events, and finishing it when the work it tracks is complete. One or more tracer objects can be created in a process through the tracer provider, a factory interface that allows for multiple 'Tracer's to be instantiated in a single process with different options.
...
{- | Create a 'Span'.
 ...
 Note: if the @tykeagent-sdk@ or another SDK is not installed, all actions that use the created
